...
public class CurrencyConversionController {
	...
	public CurrencyConversionBean getCurrencyConvertedValue(@PathVariable String from,
		...
		/*
		 * System.out.println(exchangeServiceBaseURL); logger.info("{}",
		 * exchangeServiceBaseURL);
		 */
		
		ResponseEntity<CurrencyConversionBean> respEntity =  
				new RestTemplate().getForEntity(exchangeServiceBaseURL + 
	 ...
	 * CurrencyConversionBean getCurrencyConvertedValueFeign(@PathVariable String
	 * from,
	 * 
	 * @PathVariable String to, @PathVariable BigDecimal quantity) {
	 * 
	 * CurrencyConversionBean response =
	 * currencyExchangeProxy.getCurrencyExchangeValue(from, to);
	 * 
	 * logger.info("{}", response);
	 * 
